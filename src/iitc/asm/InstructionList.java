...
public class InstructionList implements Opcodes, Iterable<AbstractInsnNode> {
    ...
    public FieldInsnNode field(Primitive descriptor, int length) {
        for (AbstractInsnNode node : this)
            if (node instanceof FieldInsnNode && NodeAnalyser.descriptorMatch((FieldInsnNode) node, descriptor, length))
                return (FieldInsnNode) node;
        return null;
    }

    public List<FieldInsnNode> fields(Primitive descriptor, int length) {
    ...
    public FieldInsnNode field(Class<?> descriptor, int length) {
        for (AbstractInsnNode node : this)
            if (node instanceof FieldInsnNode && NodeAnalyser.descriptorMatch((FieldInsnNode) node, descriptor, length))
                return (FieldInsnNode) node;
        return null;
    ...
    public List<FieldInsnNode> fields(Class<?> descriptor, int length) {
        List<FieldInsnNode> fields = new ArrayList<>(size());
        for (AbstractInsnNode node : this)
            if (node instanceof FieldInsnNode && NodeAnalyser.descriptorMatch((FieldInsnNode) node, descriptor, length))
                fields.add((FieldInsnNode) node);
    ...
    public FieldInsnNode field(Primitive descriptor) {
        for (AbstractInsnNode node : this)
            if (node instanceof FieldInsnNode && NodeAnalyser.descriptorMatch((FieldInsnNode) node, descriptor))
                return (FieldInsnNode) node;
        return null;
    }

    public List<FieldInsnNode> fields(Primitive descriptor) {
        List<FieldInsnNode> fields = new ArrayList<>(size());
        for (AbstractInsnNode node : this)
            if (node instanceof FieldInsnNode && NodeAnalyser.descriptorMatch((FieldInsnNode) node, descriptor))
                fields.add((FieldInsnNode) node);
    ...
    public FieldInsnNode field(Class<?> descriptor) {
        for (AbstractInsnNode node : this)
            if (node instanceof FieldInsnNode && NodeAnalyser.descriptorMatch((FieldInsnNode) node, descriptor))
                return (FieldInsnNode) node;
        return null;
    ...
    public List<FieldInsnNode> fields(Class<?> descriptor) {
        List<FieldInsnNode> fields = new ArrayList<>(size());
        for (AbstractInsnNode node : this)
            if (node instanceof FieldInsnNode && NodeAnalyser.descriptorMatch((FieldInsnNode) node, descriptor))
                fields.add((FieldInsnNode) node);
    ...
    public <A extends AbstractInsnNode> A type(Class<A> type, ASMCondition<A>... conditions) {
        for (AbstractInsnNode node : this)
            if (node.getClass().equals(type)) {
                A cast = (A) node;
                for (ASMCondition<A> a : conditions)
                    if (!a.validate(cast))
                        return null;
                return cast;
            }
    ...
    public <A extends AbstractInsnNode> List<A> types(Class<A> type, ASMCondition<A>... conditions) {
